-- Newsletter Advanced Features Migration
...
CREATE TABLE IF NOT EXISTS newsletter_schedules (
  ...
  send_time TEXT NOT NULL, -- HH:MM format
  day_of_week INTEGER, -- 0-6 for weekly (0=Sunday)
  day_of_month INTEGER, -- 1-31 for monthly
  template_id INTEGER,
  article_count INTEGER DEFAULT 5,
  segment TEXT, -- JSON array of category preferences to target
...
CREATE INDEX idx_schedules_next_send ON newsletter_schedules(next_send_at, is_active);
...
INSERT INTO newsletter_templates (name, description, type, subject_template, layout) VALUES
...
CREATE TABLE IF NOT EXISTS newsletter_sends (
  ...
  FOREIGN KEY (schedule_id) REFERENCES newsletter_schedules(id) ON DELETE SET NULL
);

CREATE INDEX idx_sends_date ON newsletter_sends(sent_at);
...
CREATE TABLE IF NOT EXISTS newsletter_events (
  ...
  article_id TEXT, -- For click events
  ip_address TEXT,
  user_agent TEXT,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX idx_events_send ON newsletter_events(send_id, event_type);
CREATE INDEX idx_events_email ON newsletter_events(subscriber_email, event_type);
CREATE INDEX idx_events_date ON newsletter_events(created_at);
...
CREATE TABLE IF NOT EXISTS newsletter_drip_emails (
  ...
  delay_days INTEGER NOT NULL, -- Days after trigger
  delay_hours INTEGER DEFAULT 0, -- Additional hours
  is_active BOOLEAN DEFAULT 1,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (campaign_id) REFERENCES newsletter_drip_campaigns(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS newsletter_drip_tracking (
  ...
  sent_at TIMESTAMP,
  FOREIGN KEY (campaign_id) REFERENCES newsletter_drip_campaigns(id) ON DELETE CASCADE,
  FOREIGN KEY (email_id) REFERENCES newsletter_drip_emails(id) ON DELETE CASCADE
);

CREATE INDEX idx_drip_tracking_scheduled ON newsletter_drip_tracking(scheduled_for, status);

-- Create newsletter archive table
CREATE TABLE IF NOT EXISTS newsletter_archives (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  send_id INTEGER NOT NULL UNIQUE,
  title TEXT NOT NULL,
...
CREATE INDEX idx_archives_published ON newsletter_archives(published_at);
CREATE INDEX idx_archives_slug ON newsletter_archives(slug);

-- Insert default welcome drip campaign
INSERT INTO newsletter_drip_campaigns (name, trigger_event) VALUES 
('Welcome Series', 'subscribe');

INSERT INTO newsletter_drip_emails (campaign_id, sequence_number, subject, html_content, delay_days) VALUES
(1, 1, '👋 Welcome to PKMedia!', 
